...
namespace Hazel {
	...
	Ref<Texture2D> Texture2D::CreateDepth(uint32_t width, uint32_t height, bool bLinear)
	{
		switch (Renderer::GetAPI())
		{
		case RendererAPI::API::None:    HZ_CORE_ASSERT(false, "RendererAPI::None is currently not supported!"); return nullptr;
		case RendererAPI::API::OpenGL:  return CreateRef<OpenGLTexture2D>(bLinear,width,height);
		}
